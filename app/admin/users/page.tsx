...
export default function AdminUsersPage() {
  ...
  return (
    <AdminShell>
      ...
      {error && <ErrorState title="Users could not be loaded" message={error} onRetry={() => {
        setLoading(true);
        setError('');
        load();
      }} />}
      ...
      {!loading && !error && filtered.length === 0 ? (
        <EmptyState title="No users match these filters" message="Clear the search or role filter to review all accounts." />
      ) : !loading && !error ? (
